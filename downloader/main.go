...
func main() {
    ...
    // Added generic target API flag to pick up ANY specific endpoint name requested by the user
    ...
    flag.Usage = func() {
        ...
        fmt.Printf("  go run ./downloader/main.go -mode=nse -api=historical-chart-data IMFA (Target any NSE endpoint)\n")
        fmt.Printf("  go run ./downloader/main.go -mode=bse -api=corporate-header IMFA       (Target any BSE endpoint)\n")
    }
    ...
    if len(args) >= 1 {
        ...
        // Manual scan for trailing misplaced flag strings like "-mode=bse" or "-api=xxx"
        for _, customArg := range args[1:] {
            if strings.HasPrefix(customArg, "-mode=") {
                *modePtr = strings.Split(customArg, "=")[1]
                ...
                // 🎯 FIXED: Cleanly intercepts trailing inline API arguments passed by the execution parser
                *apiPtr = strings.Split(customArg, "=")[1]
            } else if customArg == "-mode" || customArg == "-api" {
    ...
    targetApi := strings.TrimSpace(*apiPtr) // 🎯 FIXED: Holds whichever clean custom endpoint string was passed
    startTime := time.Now()
    ...
    if mode == "nse" || mode == "both" {
        ...
        // 🎯 FIXED: Direct passing of the selected API string straight into NSE environment
        if err := scrape_nse.ExecuteAll(ticker, workerCount, targetApi); err != nil {
    ...
    if mode == "bse" || mode == "both" {
        ...
        // 🎯 FIXED: Direct passing of the selected API string straight into BSE environment
        if err := scrape_bse.ExecuteAll(ticker, workerCount, targetApi); err != nil {
