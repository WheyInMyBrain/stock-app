// downloader-rust/src/nse/download.rs
use std::fs;
use std::path::{Path, PathBuf};

/// Generates the directory hierarchy structure: global_data_dir/{symbol}/nse_{api_name}
pub fn build_save_directory(global_data_dir: &Path, symbol: &str, api_name: &str) -> Result<PathBuf, String> {
    let exchange_folder = format!("nse_{}", api_name);
