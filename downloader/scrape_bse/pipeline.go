...
// Added globalDataDir to signature to handle explicit path injection cleanly from main.go
func executeStrategy(client *BSEClient, symbol, scripCode string, endpoint BSEFilingsEndpoint, workerCount int, globalDataDir string) error {
    ...
    if globalDataDir != "" {
        ...
        if err := os.MkdirAll(outputDir, 0755); err != nil {
            return fmt.Errorf("failed creating explicit global BSE target directory: %w", err)
        }
    } else {
